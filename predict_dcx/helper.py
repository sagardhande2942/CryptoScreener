...
def predict_bid_logic():
    while True:
        ...
        for i in coins:
            my_instance = prediction.objects.filter(
                coin_name=i.coin_name).first()
            pred = prediction_logs.objects.filter(
                coin_name=my_instance).first()
...
def store_updates(pred, coin):
    ...
    if last_object == None:
        prediction_counter(
            coin_name=coin, latest_price=pred.last_price, max_price=pred.last_price).save()
    else:
        ...
        if latest_price > max_price:
            ...
            cnt1 += 1
            last_object1.update(cnt=cnt, cnt1=cnt1, max_price=max_price,
                                latest_price=latest_price, last_updated=datetime.now())

        if latest_price < max_price:
            cnt -= 1
            last_object1.update(cnt=cnt, cnt1=cnt1,
                                max_price=max_price, latest_price=latest_price)
